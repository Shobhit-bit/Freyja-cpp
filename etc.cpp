...
int main(){
    int w,h,comp;
    uint8_t* img = stbi_load("data/ch2_sample3_STB.jpg",&w,&h,&comp,4);
    if(!img){ return 1; }
    ...
    Etc::File etcFile("image.ktx",Etc::File::Format::KTX,etcFormat,image.GetEncodingBits(),image.GetEncodingBitsBytes(),image.GetSourceWidth(),image.GetSourceHeight(),
