...
int main() {
    ...
    GLFWwindow* window = glfwCreateWindow(800, 600, "cube_tex", nullptr, nullptr);
    if (!window) { glfwTerminate(); return 1; }
    glfwMakeContextCurrent(window);
    if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)) return 1;
 
    // stbi_write_hdr in cube_tex.cpp writes to a relative path
    std::filesystem::create_directories("data/out");
 
    GLuint tex = loadCubemap("");
    std::printf("cubemap texture id: %u\n", tex);
